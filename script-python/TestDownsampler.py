...
#JSVM DownConvert
def DownConvert(win, hin, inyuv, wout, hout, outyuv, method=0):
  jsvmDownConvert = DOWN_CONVERT_PATH + 'DownConvert'

  cmdline = str('%s %s %s %s %s %s %s %s' %(jsvmDownConvert, win, hin, inyuv, wout, hout, outyuv, method))
...
#wels downsampler
def downsampler(win, hin, inyuv, wout, hout, outyuv, method=0):
  welsDownsampler = DOWN_SAMPLER_PATH + 'downsampler.bin'

  cmdline = str('%s %s %s %s %s %s %s -method %s' %(welsDownsampler, win, hin, inyuv, wout, hout, outyuv, method))
...
#wels H264encoder
def h264enc(outfile, infile, win, hin, wout, hout, qp=24, rc=-1):
  welsencoder = H264ENC_PATH + 'h264enc'
  welscfgfile = H264CFG_PATH + 'welsenc.cfg'
  welslayerfile = H264CFG_PATH + 'layer2.cfg'

  cmdline = str('%s %s -lconfig 0 %s -bf %s -org %s -RCMode -1 -sw %s -sh %s -RCMode -1 -dw 0 %s -dh 0 %s -lqp 0 %d'
                % (welsencoder, welscfgfile, welslayerfile, outfile, infile, win, hin, wout, hout, qp))
  #print cmdline
  p = subprocess.Popen(cmdline, shell=True, stdout=subprocess.PIPE)
  result_line = p.communicate()[0]
  #print result_line
...
#wels H264decoder
def h264dec(infile, outfile):
  welsdecoder = H264DEC_PATH + 'h264dec'
  cmdline = str('%s %s %s' % (welsdecoder, infile, outfile))
