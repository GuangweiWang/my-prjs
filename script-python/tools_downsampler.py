'''
define following downsamplers for common use:
    - jsvm_down_convert(win, hin, inyuv, wout, hout, outyuv)
    - wels_downsampler(win, hin, inyuv, wout, hout, outyuv, method=0)
    - scalerSX80(inbmp, wout, hout)
'''
...
def jsvm_down_convert(win, hin, inyuv, wout, hout, outyuv, method=0):
    ...
    parameters:
        ...
        method          optional parameters, default 0,
...
def wels_downsampler(win, hin, inyuv, wout, hout, outyuv, method=0):
    ...
    downsampler = DOWN_SAMPLER_EXE_PATH + 'down_sampler.bin'

    cmdline = str('%s %s %s %s %s %s %s -method %d' %(downsampler, win, hin, inyuv, wout, hout, outyuv, method))
...
def scalerSX80(in_bmp, wout, hout):
    ...
    parameters:
        ...
        hout            the height of output file
    return:
        no parameters returned, but save the following two files:
